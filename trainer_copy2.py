...
@LastEditors: houwx
@LastEditTime : 2020-02-13 15:23:00
@Description: 
'''
import time
...
class Trainer(object):
    ...
    def build_model(self):
        ...
        if self.mode == "vqvae" or self.mode == "vqvae1":
                ...
                for epoch in range(1, self.hps.melgan_epochs + 1):
                    ...
                    for iterno, (x, speaker_info) in enumerate(train_data_loader):
                        ...
                        if self.num_speaker != -1:
                            ...
                            # spk_embed_params = torch.cat([x.view(-1) for x in self.spk_embed.parameters()])
                            # print(speaker_id)
                            # print(self.spk_embed.weight.grad)
                            # print(spk_embed_params)
                        ...
                        # spk_embed_params = torch.cat([x.view(-1) for x in self.spk_embed.parameters()])
                        # l2_regularization = torch.norm(spk_embed_params, 2)
                        # print(l2_regularization)
                        # mel_error += l2_regularization
...
if __name__ == "__main__":
    ...
    #ckpt_path = "./ckpts_codebook128_mel256_spk_adv_enc_rec_loss/"
    ...
    if not os.path.exists(args.ckpt_path + "logs/"):
        os.makedirs(args.ckpt_path + "logs/")
    ...
    if args.mode == "vqvae":
        ...
        trainer = Trainer(
            hps=hps, 
            logger_path=args.ckpt_path + "logs/",
    ...
    if args.load_melgan:
        name = 'melgan' 
        trainer.load_model(args.ckpt_path + 'model-melgan-1000-220-220999-False.pt', name)
