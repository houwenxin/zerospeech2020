...
import random # For joint training
...
class Trainer(object):
    ...
    def build_model(self):
        ...
        if self.mode == "joint":
            from model.vqvae.vqvae_encoder import VQVAE_Encoder as VQVAE
            # Re-define VQVAE
            self.vqvae = VQVAE(in_channel=80, channel=512, 
                                embed_dim=self.hps.vqvae_embed_dim, 
                                n_embed=self.hps.vqvae_n_embed,
    ...
    def train(self, save_model_path, speaker2id=None):
        '''
        speaker2id is for joint training (selecting target speaker randomly).
        '''
        total_iterno = -1
        if self.mode == "vqvae":
            loss_rec_best = 10000
            ...
            for epoch in range(1, self.hps.melgan_epochs + 1):
                ...
                for iterno, (x, speaker_info) in enumerate(train_data_loader):
                    ...
                    if self.num_speaker != -1:
                        speaker_id = speaker_info['source_speaker']['id'].to(self.device) # Rec to the source speaker in this step.
        ...
        elif self.mode == "joint": # Proposed Method: Self-Cyclic Speaker-Independent Voice Conversion for ZeroSpeech 2020
            assert speaker2id is not None, "Speaker id should be given during joint training."

            loss_rec_best = 10000
            costs = []
            start = time.time()
            target_speakers = ['V001', 'V002']
            for epoch in range(1, self.hps.vqvae_epochs + 1):
                train_data_loader = tqdm(self.train_data_loader, total=len(self.train_data_loader))
                for iterno, (x, speaker_info) in enumerate(train_data_loader):
                    self.vqvae.train() # Set to train mode.
                    ...
                    x = x.to(self.device)
                    x_mel = self.audio2mel(x).detach()
                    x_enc = self.vqvae(x_mel)
                    if self.num_speaker != -1:
                        source_speaker = speaker_info['source_speaker']['name']
                        target_speaker = random.choice(target_speakers) # choose one target randomly
                        src_speaker_id = speaker_info['source_speaker']['id'].to(self.device) # Should be reconst or convert within a probability (0.5).
                        trg_speaker_id = speaker2id[target_speaker]
...
if __name__ == "__main__":
    ...
    if mode == "vqvae":
        ...
        train_dataset = AudioDataset(audio_files=Path(data_path) / "rec_train_files.txt", segment_length=hps.seg_len, sampling_rate=16000, mode='reconst')
